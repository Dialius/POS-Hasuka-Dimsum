...
> ⚠️ **JANGAN AI SLOP** — khusus dokumen ini: jangan implementasi "offline mode" versi tempelan (mis. cuma disable tombol pas offline, atau tampilkan spinner "menunggu koneksi"). Ini harus benar-benar **local-first**: aplikasi tidak boleh tahu/peduli status koneksi untuk bisa menyelesaikan transaksi.
...
- Listrik padam (device harus tetap hidup dari baterai — lihat §6)
...
Background Sync Worker (thread/proses terpisah):
   ...
   - Setelah push sukses, tarik (pull) update produk/harga/**aturan promo**/**pengaturan pajak (PPN on/off & rate)** terbaru dari server — lihat PRD-03 §6 untuk endpoint-nya
```
...
- **Stok**: dikurangi optimistic di local DB saat transaksi terjadi. Saat sync, server jadi otoritas final — kalau ternyata terjadi oversell lintas terminal (dua kasir di outlet sama menjual item terakhir yang sama saat sama-sama offline), server menandai transaksi kedua untuk **review manual**, bukan otomatis dibatalkan begitu saja (barang sudah keluar & uang sudah diterima secara fisik).
- **Harga & data produk**: server adalah otoritas. Kalau owner ubah harga saat outlet offline, harga lama tetap dipakai sampai terminal itu online lagi dan pull update — ini trade-off yang diterima (transaksi tetap sah dengan harga yang berlaku saat itu di device tsb).
...
| Perhitungan pajak (PPN) | ✅ Ya | Pakai `tax_enabled`/`tax_rate` hasil pull terakhir — kalau owner ubah pengaturan pajak saat terminal offline, terminal itu tetap pakai rate lama sampai online & pull ulang (sama seperti kasus harga di §4) |
| QRIS statis (stiker/poster) | ✅ Ya | **Tidak butuh device online sama sekali** — lihat §7 |
| QRIS dinamis | ❌ Tidak | Butuh request ke server payment gateway tiap generate kode |
| Kartu via EDC bank | ⚠️ Tergantung EDC | EDC biasanya punya jalur sendiri (SIM card sendiri), independen dari internet toko |
...
- Device terminal sebaiknya **laptop/tablet dengan baterai**, bukan PC desktop tanpa UPS — supaya transaksi yang sedang berjalan tidak hilang begitu listrik padam mendadak.
- Kalau pakai PC desktop, sediakan **UPS minimal untuk PC + printer** cukup untuk menyelesaikan transaksi berjalan dan shutdown dengan aman (bukan untuk operasi jangka panjang).
...
## 7. Static QRIS sebagai Fallback Pembayaran Non-Tunai Saat Total Offline

Ini teknik yang sengaja dipakai beberapa bisnis F&B chain: pakai **QRIS dinamis** di kasir utama untuk transaksi cepat & tercatat otomatis, tapi tetap sediakan **QRIS statis tercetak/stiker** di meja kasir sebagai cadangan kalau sistem POS down. QRIS statis tidak butuh listrik atau internet di sisi merchant sama sekali — pelanggan yang scan dan input nominal sendiri lewat aplikasi mereka. Konsekuensinya: transaksi ini **tidak otomatis tercatat di sistem POS**, jadi perlu proses rekonsiliasi manual dari dashboard payment gateway saat kembali online. Ini trade-off yang wajar untuk kondisi darurat (POS/internet total down), bukan alur utama.

## 8. Skenario Uji Wajib

- [ ] Cabut internet di tengah transaksi → transaksi selesai lokal → struk tercetak
- [ ] Restart aplikasi mendadak (simulasi mati listrik) di tengah transaksi → tidak ada data korup, transaksi terakhir yang belum selesai tidak nyangkut setengah-setengah
- [ ] Offline lama (>1 hari), lalu online lagi → semua transaksi yang menumpuk berhasil sync tanpa duplikat, tanpa urutan tercampur
