...
import AddEditProductModal, { type Product } from './AddEditProductModal'
import { PRODUCTS, RECIPES, INGREDIENTS } from '../data/mockData'
...
function recipeStockEstimate(productId: number): { min: number; unit: string } | null {
  const recipes = RECIPES.filter(r => r.product_id === productId)
...
export default function ManageProductsScreen({ onBack, backLabel, onNavigate }: { onBack: () => void; backLabel?: string; onNavigate?: (s: string) => void }) {
  ...
  const marginPct = selected.price > 0 ? ((margin / selected.price) * 100).toFixed(1) : '0'
  const estimate = selected.stock_mode === 'recipe' ? recipeStockEstimate(selected.id) : null
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          {filtered.map((p, i) => {
            ...
            const est = p.stock_mode === 'recipe' ? recipeStockEstimate(p.id) : null
            return (
