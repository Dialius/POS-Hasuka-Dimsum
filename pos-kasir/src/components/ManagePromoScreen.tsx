...
import { Plus, Tag, Calendar, X, Trash2, Store } from 'lucide-react'
    ...
    <PageShell
      ...
      headerRight={
        <button
          ...
          className="flex items-center gap-2 px-4 sm:px-4 py-2 rounded-xl font-bold text-[12px] sm:text-[13px] shadow-lg transition-all hover:shadow-xl active:scale-95"
          style={{
      ...
      rightPanel={
        selected ? (
          <div className="flex flex-col h-full">
            <div className="px-6 py-6 shrink-0" style={{ borderBottom: '1px solid #E8D7C0', background: '#F3E7CE' }}>
              <span className="text-[11px] font-bold px-3 py-1 rounded-full inline-block mb-2.5 shadow-sm" style={{ background: sc.bg, color: sc.color }}>
                {selected.status}
              </span>
            ...
            <div className="px-6 py-4 flex-1 overflow-y-auto">
              <div className="space-y-3 mb-6">
                {[
                  ...
                  const Icon = r.Icon
                  return (
                    <div key={r.label} className="flex items-start gap-4 p-4 rounded-2xl bg-white border border-[#E8D7C0] shadow-sm">
                ...
                {selected.type === 'bundling' && selected.bundleProducts && selected.bundleProducts.length > 0 && (
                  <div className="rounded-2xl p-3.5 bg-white border border-[#E8D7C0]">
                    <p className="text-[10px] font-bold mb-2" style={{ color: '#8B4A1E' }}>ISI PAKET BUNDLE:</p>
          ...
          <div className="p-6 h-full flex flex-col items-center justify-center text-center text-[#6B5448]">
            ...
            <p className="text-[12px] mt-1 opacity-70 mb-6 max-w-[220px]">
    ...
    >
        ...
        <div className="bg-white rounded-2xl border border-[#E8D7C0] overflow-hidden">
          {promosList.length === 0 ? (
            <div className="p-10 text-center flex flex-col items-center justify-center">
              <div className="w-12 h-12 rounded-2xl flex items-center justify-center mb-4" style={{ background: '#F3E7CE' }}>
                <Tag size={24} color="#8B4A1E" />
              </div>
              <p className="font-bold text-[15px] text-[#2B1810] mb-1">Belum Ada Promo</p>
              <p className="text-[12px] text-[#6B5448] mb-6 max-w-sm">
      ...
      {selected && (
        <ConfirmDialog
          ...
          variant="destructive"
          title="Hapus Promo?"
          description={`Promo "${selected.name}" akan dihapus permanen dari semua cabang. Tindakan ini tidak dapat dibatalkan.`}
