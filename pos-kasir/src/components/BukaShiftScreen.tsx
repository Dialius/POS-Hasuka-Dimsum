...
export default function BukaShiftScreen({ onBukaShift }: { onBukaShift: () => void }) {
  ...
  return (
    <div className="flex flex-col md:flex-row w-full h-full overflow-hidden" style={{ background: '#FAF6ED' }}>
      ...
      {/* Left: context + info */}
      <div className="flex flex-col flex-1 overflow-y-auto px-4 md:px-10 py-5 md:py-8 md:border-r" style={{ borderColor: '#E8D7C0' }}>

        {/* Brand mark */}
        <div className="flex items-center gap-3 md:gap-4 mb-4 md:mb-8">
          <img src={HASUKA_LOGO} alt="Hasuka" className="w-10 h-10 md:w-12 md:h-12 object-contain rounded-full" />
          <div>
            <h1 className="font-serif font-bold text-[18px] md:text-[22px] leading-tight" style={{ color: '#2B1810' }}>Hasuka POS</h1>
            <p className="text-[11px] md:text-[12px]" style={{ color: '#6B5448' }}>Sistem Kasir · {outlet.name}</p>
          </div>
        </div>
        ...
        {hasPreviousShift && previousShift && (
          <div
            className="flex items-start gap-3 p-3.5 md:p-4 rounded-xl md:rounded-2xl mb-4 md:mb-6"
            style={{ background: '#FCE8E8', border: '1px solid rgba(182,0,0,0.2)' }}
          >
            <AlertTriangle size={18} color="#B60000" className="shrink-0 mt-0.5" strokeWidth={2.5} />
            <p className="text-[12px] md:text-[13px] font-semibold leading-relaxed" style={{ color: '#B60000' }}>
        ...
        {/* Big title */}
        <h2 className="font-serif font-bold text-[22px] md:text-[30px] leading-tight mb-1 md:mb-2" style={{ color: '#2B1810' }}>
          Buka Shift
        </h2>
        <p className="text-[13px] md:text-[14px] mb-4 md:mb-6" style={{ color: '#6B5448' }}>
          Modal awal laci kasir sebelum mulai transaksi.
        </p>

        {/* Info box */}
        <div className="rounded-xl md:rounded-2xl p-4 md:p-5 mb-4 md:mb-6" style={{ background: '#F3E7CE', border: '1px solid #E8D7C0' }}>
          <div className="space-y-2.5">
            {[
              ...
              <div key={row.label} className="flex justify-between items-center text-[12px] md:text-[13px]">
        ...
        {/* Notes */}
        <div>
          <label htmlFor="catatan-shift" className="block text-[11px] md:text-[12px] font-bold mb-1.5" style={{ color: '#6B5448', letterSpacing: '0.04em' }}>
            CATATAN (OPSIONAL)
          </label>
          <textarea
            id="catatan-shift"
            ...
            className="w-full p-3.5 rounded-xl text-[13px] md:text-[14px] resize-none outline-none transition-colors"
            style={{
              background: 'white',
      ...
      <div
        className="flex flex-col shrink-0 px-4 md:px-7 py-5 md:py-8 w-full md:w-[350px] lg:w-[380px] overflow-y-auto"
        style={{ background: '#F3E7CE', borderBottom: '1px solid #E8D7C0' }}
      >
        <p className="text-[10px] md:text-[11px] font-bold mb-1.5" style={{ color: '#6B5448', letterSpacing: '0.08em' }}>
          KAS AWAL
        </p>

        {/* Nominal display */}
        <div
          className="rounded-xl md:rounded-2xl p-4 md:p-5 mb-4 flex items-end gap-2"
          style={{
            background: 'white',
        ...
        >
          <span className="font-bold text-[16px] md:text-[18px]" style={{ color: '#6B5448' }}>Rp</span>
          <span className="font-serif font-bold text-[26px] md:text-[32px] leading-none" style={{ color: '#2B1810' }}>
            {displayNominal}
          </span>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 mb-4">
          {[50000, 100000, 200000, 300000, 500000, 1000000].map(amt => (
            <button
              ...
              className="py-2 rounded-xl text-[11px] md:text-[12px] font-bold transition-colors"
              style={{
        ...
        <div className="grid grid-cols-3 gap-2 mb-4">
          {[1, 2, 3, 4, 5, 6, 7, 8, 9].map(n => (
            <button
              ...
              className="transition-all active:scale-95 disabled:opacity-50 font-bold text-[18px] md:text-[20px] rounded-xl min-h-[46px] md:min-h-[54px]"
              style={{
          ...
          <button
            ...
            disabled={isSubmitting}
            className="rounded-xl flex items-center justify-center transition-all active:scale-95 disabled:opacity-50 min-h-[46px] md:min-h-[54px]"
            style={{ 
          ...
          <button
            ...
            aria-label="Tiga nol"
            disabled={isSubmitting}
            className="transition-all active:scale-95 disabled:opacity-50 font-bold text-[15px] md:text-[16px] rounded-xl min-h-[46px] md:min-h-[54px]"
            style={{
              background: 'linear-gradient(180deg, #ffffff 0%, #f9f9f9 100%)',
              color: '#2B1810',
              border: '2px solid #E8D7C0',
        ...
        {errorMsg && (
          <div role="alert" className="mb-3 p-3.5 rounded-xl bg-red-50 border border-red-200">
        ...
        <Button
          ...
          className="py-3.5 md:py-4 text-[15px] md:text-[16px] shadow-lg hover:shadow-xl"
          style={{
        ...
        <p className="text-center text-[10px] md:text-[11px] mt-3" style={{ color: '#6B5448' }}>
