// Deteksi Tauri di level modul agar import plugin tidak dieksekusi di browser biasa.
// Import statis '@tauri-apps/plugin-sql' di browser menyebabkan error saat load bundle.
const isTauri = typeof window !== 'undefined' && '__TAURI_INTERNALS__' in window;

// Tipe minimal untuk menghindari `any` (TS2347 pada generic call)
interface SqlDb {
  load(path: string): Promise<SqlDb>
...
async function loadDb(): Promise<SqlDb> {
  if (!isTauri) throw new Error('SQLite hanya tersedia di aplikasi desktop Tauri');
  const mod = await import('@tauri-apps/plugin-sql');
  return mod.default as unknown as SqlDb;
}

export const initDb = async (): Promise<SqlDb> => {
  if (!db) {
...
export const getDb = async (): Promise<SqlDb> => {
  if (!db) {
    return await initDb();
  }
  return db;
